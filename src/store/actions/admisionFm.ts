...
export const formatDataValidated = (status: number, aci: number, validado: any) => ({
	...
	valids: {
		...
		//msg
		valid_client:
			!validado.valid_cliente.status && validado.valid_cliente.id_typedif === 2 ? validado.valid_cliente.msg : '',
		valid_commerce:
...
export const updateStatusFM = (
	id_fm: number,
	status: any,
	validado: ValidatedFace,
	aci: number,
	socket: Socket,
	setFm: any,
	setModelOpen: any
) => {
	const id_status: any = formatDataValidated(status, aci, validado);
	//console.log(id_fm, status);
	return async (dispatch: any) => {
		try {
			...
			//todo ok
			Swal.close();
			await successValidationFM(status, socket, setFm, setModelOpen);
