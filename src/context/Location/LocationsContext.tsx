...
const LocationsProvider = (props:any) => {
	...
	//Estado
	const setListEstadoClient = (estados: any) => {
		dispatchC({
			type: SET_ESTADO,
			payload: estados,
		});
	...
	const setListEstadoCommerce = (estados: any) => {
		dispatchCC({
			type: SET_ESTADO,
			payload: estados,
		});
	...
	const setListEstadoPos = (estados: any) => {
		dispatchP({
			type: SET_ESTADO,
			payload: estados,
		});
	...
	//Municipio
	const setListMunicipioClient = async (value: any) => {
		if(value){
			try{
	...
	const setListMunicipioCommerce = async (value: any) => {
		if(value){
			try{
				await axios.get(`/Location/${value.id}/municipio`).then((res:any) => {
					dispatchCC({
	...
	const setListMunicipioPos = async (value: any) => {
		if(value){
			try{
				await axios.get(`/Location/${value.id}/municipio`).then((res:any) => {
					dispatchP({
	...
	//Ciudad
	const setListCiudadClient = async (id: any) => {
		if(id){
			try{
	...
	const setListCiudadCommerce = async (id: any) => {
		if(id){
			try{
				await axios.get(`/Location/${id}/ciudad`).then((res:any) => {
					dispatchCC({
	...
	const setListCiudadPos = async (id: any) => {
		if(id){
			try{
				await axios.get(`/Location/${id}/ciudad`).then((res:any) => {
					dispatchP({
